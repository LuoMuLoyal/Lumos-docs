...
- `Lumos workspace root` is not a git repository; `Lucent` and `Luminous` are separate child repositories.
...
- Current generated client baseline after report dashboard + range-based AI summary contracts: 47 paths / 138 schemas.
...
- Report: Lucent-backed last-7-days dashboard with real medication/water aggregates, explicit insufficient-data handling for unsupported sleep data, contract-driven findings/patterns text, manual AI summary generation through `/api/v1/user/reports/summary/generate`, local signed-out/disabled/loading/success/error AI-summary states, in-card `近 7 天 / 近 30 天` AI summary switching with per-range cached state, mobile pull-to-refresh plus explicit sync action, signed-out in-tab placeholder plus login notice instead of a full-page error gate, frontend-only export placeholders, and reference notice. Privacy settings are owned by Mine/Settings.
